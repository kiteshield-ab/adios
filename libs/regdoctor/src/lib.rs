...
pub fn for_each_register(
    device: svd_rs::Device,
    mut for_each_register: impl FnMut((RegisterInfo, u64)),
) {
    let device = helpers::expand_derived_from_attribute(device);
    for peripheral in device.peripherals.into_iter() {
        ...
        for register in peripheral.registers() {
            ...
            for_each_register((register_info, address))
        }
...
impl Database {
    ...
    pub fn extend_with_svd(&mut self, device: svd_rs::Device) {
        for_each_register(device, |(register_info, address)| {
            let identifier = register_info.identifier();
            match self.regs.insert(address, register_info) {
                Some(previous) => {
                    log::info!(
