use crate::adi::{Command, Timestamp};
use bilge::prelude::*;
use nom::{
    branch::alt,
    bytes::complete::tag,
...
pub fn file_commands(input: &str) -> Result<Vec<Command>, nom::Err<nom::error::Error<&str>>> {
...
";
        let commands = file_commands(text_sample).unwrap();
